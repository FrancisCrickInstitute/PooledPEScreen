"""
    Project :  Prime editing pilot screens. Plotting script to generate Figure 4 b. Stripplot of function scores (variant-level scores) by ClinVar pathogenicity annotation. MLH1 non-coding screen.
    Date : 240316
    Python version 3.10
...
def stripplot(
    ...
    df = df[
        df["ClinVar_Significance_Simple"].isin(
            [
                "Benign / Likely benign",
                "Pathogenic / Likely pathogenic",
                "Uncertain / Conflicting",
...
def main():
    ...
    save_path = save_dir / (f"Fig_4b_clinvar_strip_plot.svg")
    ...
    variants = df_variant_score_filtered[
        "log2_mean_variant_replicates_normalised"
    ].count()
    print(f"number of variants scored is {variants}")
    ...
    print(f"number of variants scored post dropna is {variants}")
