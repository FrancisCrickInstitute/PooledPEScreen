"""
    Project :  Prime editing pilot screens. Plotting script to generate Figure 4 e. Scatterplot: function scores (variant scores) over positions (GRCh38) for upstream region. MLH1 non-coding screen.
    Date : 240316
    Python version 3.10
...
def main():
    """
    Main function to plot Figure 4 e. Scatterplot: function scores (variant-level scores) over positions (GRCh38) for upstream region, with both "consequence" and "ClinVar" color-coding. MLH1 non-coding screen.
    """
    save_dir = OUTPUT_DIR
    save_dir.mkdir(exist_ok=True, parents=True)
    save_path_consequence = save_dir / (
        f"Fig_4e_plot_position_scatter_function_score_upstream_consequence.svg"
    )
    save_path_clinvar = save_dir / (
        f"Fig_4e_plot_position_scatter_function_score_upstream_clinvar.svg"
    )
