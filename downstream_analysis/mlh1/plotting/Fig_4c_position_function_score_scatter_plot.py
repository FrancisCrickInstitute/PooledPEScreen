"""
    Project :  Prime editing pilot screens. Plotting script to generate Figure 4 c. Scatterplot: function scores (variant scores) over positions (GRCh38). MLH1 non-coding screen.
    Date : 240316
    Python version 3.10
...
def position_scatterplot_non_coding(
    df, hue_order, x, y, hue, naming_dict, save_path=None
):
    """
...
def main():
    """
    Main function to plot Figure 4 c for PE manuscript. Scatterplot: functions scores (variant scores) over positions (GRCh38). MLH1 non-coding screen.
    """
    save_dir = OUTPUT_DIR
    save_dir.mkdir(exist_ok=True, parents=True)
    save_path_consequence = save_dir / (
        f"Fig_4c_plot_position_scatter_function_score.svg"
    )
