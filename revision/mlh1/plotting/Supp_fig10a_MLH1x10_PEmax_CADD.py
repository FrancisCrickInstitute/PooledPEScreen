"""
    Project :  Prime editing pilot screens. Revisions. Plotting script to generate Supplementary Figure 10a using only data generated in the HAP1:PEmax cell line. Correlation Scatterplot: functions scores (variant scores) against Cadd (Phred) scores. MLH1x10 screen.
    Date : 240917
    Python version 3.10
...
def Correlation_scatter(df, x, y, hue, hue_order, ax, save_path=None):
    ...
    variants = df["log2_mean_variant_PEmax_normalised"].count()
...
def main():
    """
    Main function to plot Supplementary Figure 10a using only Function Score generated in PEmax. Scatterplot: functions scores (variant scores) against CADD scores for both relaxed and high-stringency datasets. MLH1x10 screen.
    """
    save_dir = OUTPUT_DIR
    save_dir.mkdir(exist_ok=True, parents=True)

    save_path = save_dir / (
        f"Supp_Fig_10a_plot_replicates_cadd_score_correlation_PEmax_only_no_stars.svg"
    )
