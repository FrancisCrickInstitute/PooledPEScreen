"""
    Project :  Prime editing pilot screens. Script to generate a heatmap to visualise score differences across cell lines.
    Date : 241113
    Python version 3.10
"""
...
supp_path = INPUT_DIR / "Table_supp_6_MLH1_x10_variant_scores.xlsx"
...
def heatmap(df, save_path):
    ...
    ax = sns.heatmap(
        ...
        vmin=-2.77,  # Set the minimum color scale value
        vmax=11.05,  # Set the maximum color scale value
    )
...
def main():
    """
    Main pre-processing function to generate a dataframe containing variant scores for the MLH1 exon 10 screen.
    ...
    save_path_lof = save_dir / (f"MLH1x10_score_differences_heatmap.svg")
    save_path_neutral = save_dir / (f"MLH1x10_score_differences_heatmap_neutral.svg")
    ...
    heatmap(df_supp, save_path_lof)

    df_neutral = pd.read_excel(supp_path, header=1)
