"""
    Project :  Prime editing pilot screens. Plotting script to generate Supplementary Figure 10b using only data generated in the HAP1:PEmax+MLH1dn cell line. Correlation Scatterplot: functions scores (variant scores) against SpliceAI scores. MLH1 non-coding screen.
    Date : 240318
    Python version 3.10
...
def SpliceAI_correlation_scatter(
    ...
    df = df.dropna(subset="log2_mean_variant_PEmaxdn_normalised")
...
def main():
    """
    Main function to plot Supplementary Figure 10b using Function Scores generated in PEmaxdn. Scatterplot: function scores (variant-level scores) against SpliceAI scores. MLH1 non-coding screen.
    """
    save_dir = OUTPUT_DIR
    save_dir.mkdir(exist_ok=True, parents=True)
    save_path = save_dir / (
        f"Supp_Fig_10b_plot_SpliceAI_score_correlation_PEmaxdn_only_no_stars.svg"
    )
    ...
    df_PEmaxdn = df_PEmaxdn.drop_duplicates(subset=["var_key"])
    ...
    SpliceAI_correlation_scatter(
        df=df_PEmaxdn.dropna(subset="SpliceAI_max"),
        x="SpliceAI_max",
