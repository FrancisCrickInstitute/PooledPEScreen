"""
    Project :  Prime editing pilot screens. Plotting script to generate Figure 4 d. Correlation Scatterplot: functions scores (variant scores) against SpliceAI scores. MLH1 non-coding screen.
    Date : 240316
    Python version 3.10
...
df_non_coding_variant_score_path = (
    INPUT_DIR / "data_MLH1intronic_variant_score_replicates.csv"
)
...
def SpliceAI_correlation_scatter(
    ...
    """
    Plotting function generating correlation scatterplot with function scores against SpliceAI scores, marking LoF variants as stars.
    ...
    df = df.dropna(subset="log2_mean_variant_replicates_normalised")
...
def main():
    """
    Main function to plot Figure 4 d. Scatterplot: function scores (variant-level scores) against SpliceAI scores. MLH1 non-coding screen.
    """
    save_dir = OUTPUT_DIR
    save_dir.mkdir(exist_ok=True, parents=True)
    save_path = save_dir / (f"Fig_4d_plot_SpliceAI_score_correlation_no_stars.svg")
    ...
    SpliceAI_correlation_scatter(
        df=df_variant_score_filtered.dropna(subset="SpliceAI_max"),
        x="SpliceAI_max",
