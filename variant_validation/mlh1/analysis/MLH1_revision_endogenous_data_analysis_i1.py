"""
    Project :  Prime editing pilot screens. Revision experiment. Pre-processing script for endogenous MLH1 i1 data. Input are DIMSUM output dataframes containing variant counts. Output dataframes are: "i1_data_full.csv".
    Date : 240905
    Python version 3.10
...
def convert_variant_positions_to_GRCh38location_reverse(positions, offset):
    """
    ...
    for position in positions:
        locations.append((offset + 1) - position)
    return locations
...
def create_var_key_df_endogenous(df):
    ...
    df (df): Dataframe to be processed.
    """
    complement_dict = {"G": "C", "C": "G", "A": "T", "T": "A"}

    df["GRCh38location"] = df["GRCh38location"].apply(
        lambda x: x[0] if isinstance(x, list) and len(x) > 0 else None
    )
    df["GRCh38location"] = df["GRCh38location"].fillna(0).astype(int)
    df["variants"] = df["variants"].apply(
        lambda x: x[0] if isinstance(x, list) and len(x) > 0 else None
    )
    df["complement"] = df["variants"].map(complement_dict)
    df["var_key"] = (
        df["GRCh38location"].astype(str) + "_" + df["complement"].astype(str)
    )
    return df
...
def process_dimsum_RData_file(
    ...
    Parameters
    ----------
    csv_path_D19_D33 (path): Path to CSV file containing variant counts on D33 and D19.
    ...
    df_merge_full = df_D19_D33_concat.pipe(
    ...
    df_subset_correct_length["GRCh38location"] = df_subset_correct_length[
        "variant_position"
    ].apply(convert_variant_positions_to_GRCh38location_reverse, args=(offset,))
    df_subset_correct_length = df_subset_correct_length.pipe(
