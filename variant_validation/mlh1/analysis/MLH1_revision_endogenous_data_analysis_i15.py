"""
    Project :  Prime editing pilot screens. Revision experiment. Pre-processing script for endogenous MLH1 x10 data. Input are DIMSUM output dataframes containing variant counts. Output dataframes are: "i15_data_full.csv".
    Date : 240905
    Python version 3.10
...
## Negative control and D19 (variants)

sample_neg_path = INPUT_DIR / f"{wt_sample}_bq5_fitness_replicates.csv"
...
neg_indel_path = INPUT_DIR / f"{wt_sample}_bq5_indel_variant_data_merge.tsv"
...
neg_rejected_path = INPUT_DIR / f"{wt_sample}_bq5_rejected_variant_data_merge.tsv"
...
def process_negative_control(csv_path_neg_D19, df, df_indels, df_rejected):
    """
    Generate dataframe containing counts of variants, indels, and rejected variants in the negative control.

    Parameters
    ----------
    csv_path_neg_19: Path to CSV file containing negative control and D19 variant counts.
    df (df): Dataframe to be processed.
    df_indels (df): Dataframe containing negative control and D19 indel counts.
    df_rejected (df): Dataframe containing negative control and D19 counts of rejected variants.
    """
    save_dir = OUTPUT_DIR / csv_path_neg_D19.name.split(".")[0]
    save_dir.mkdir(exist_ok=True, parents=True)

    df_concat = pd.concat([df, df_rejected, df_indels], axis=0, join="outer")
    df_concat = df_concat.pipe(
        calculate_pre_and_post_frequencies, pre="neg", post="D19"
    )

    df_concat.to_csv((save_dir / f"data_neg_endogenous.csv").as_posix())
...
def process_dimsum_RData_file(
    ...
    csv_path_rejected,
    csv_path_neg_D19,
    csv_path_neg_D19_indels,
    csv_path_neg_D19_rejected,
    csv_path_non_SNV,
    df_variant_score,
    save_dir,
    ...
    """
    Processing individual endogenous samples with dimsum variant count dataframes as input (D33 vs D19, D20 vs negative control).
    ...
    csv_path_indels (path): Path to CSV file containing D33 and D19 indel counts.
    csv_path_rejected (path): Path to CSV file containing D33 and D19 counts of rejected variants.
    csv_path_neg_D19 (path): Path to CSV file containing D19 and negative control variant counts.
    csv_path_neg_D19_indels (path): Path to CSV file containing D19 and negative control indel counts.
    csv_path_neg_D19_rejected (path): Path to CSV file containing D19 and negative control counts of rejected variants.
    ...
    # Import dataframes (neg/D19, neg/D19 indels, neg/D19 rejected)
    df_neg_D19 = pd.read_csv(csv_path_neg_D19)
    df_neg_D19_indels = pd.read_csv(csv_path_neg_D19_indels, delimiter="\t")
    df_neg_D19_indels.rename(
        columns={
            f"input_e1_s0_bNA_count": "count_e1_s0",
            "output_e1_s1_b1_count": "count_e1_s1",
        },
    ...
    df_neg_D19_indels["nt_seq"] = df_neg_D19_indels["nt_seq"].str.upper()

    df_neg_D19_rejected = pd.read_csv(csv_path_neg_D19_rejected, delimiter="\t")
    df_neg_D19_rejected.rename(
        columns={
            f"input_e1_s0_bNA_count": "count_e1_s0",
            "output_e1_s1_b1_count": "count_e1_s1",
        },
    ...
    df_neg_D19_rejected["rejected"] = True
    ...
    df_neg = process_negative_control(
        csv_path_neg_D19,
        df=df_neg_D19,
        df_indels=df_neg_D19_indels,
        df_rejected=df_neg_D19_rejected,
    )

    # Merge dataframes (D19/D33 with neg)
    ...
    df_merge_indels = df_merge_indels.pipe(
        calculate_ratio_and_log2, pre="D19", post="D33"
    ).pipe(calculate_ratio_and_log2, pre="neg", post="D19")

    # Process no_indel dataframe (add ratios and log2)
    df_merge_no_indels = df_merge_no_indels.pipe(
        calculate_ratio_and_log2, pre="D19", post="D33"
    ).pipe(calculate_ratio_and_log2, pre="neg", post="D19")
...
def MLH1_endogenous_preprocessing(
    ...
    """
    Processing individual endogenous samples with dimsum variant count dataframes as input (D33 vs D19, D19 vs negative control).
    ...
    csv_path_indels (path): Path to CSV file containing D33 and D19 indel counts.
    csv_path_rejected (path): Path to CSV file containing D33 and D19 counts of rejected variants.
    csv_path_neg (path): Path to CSV file containing D19 and negative control variant counts.
    csv_path_neg_indels (path): Path to CSV file containing D19 and negative control indel counts.
    csv_path_neg_rejected (path): Path to CSV file containing D19 and negative control counts of rejected variants.
